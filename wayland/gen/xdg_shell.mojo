...
def xdg_wm_base_get_xdg_surface(self: WLPtr, surface: Pointer[NoneType, MutUntrackedOrigin]) raises -> WLPtr:
...
def xdg_wm_base_listen(self: WLPtr, out_queue: Pointer[WLPtr, MutUntrackedOrigin]) -> Int32:
...
def xdg_wm_base_next_ping(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
...
def xdg_surface_get_popup(self: WLPtr, parent: Pointer[NoneType, MutUntrackedOrigin], positioner: Pointer[NoneType, MutUntrackedOrigin]) raises -> WLPtr:
    # opcode 2: get_popup
...
def xdg_surface_listen(self: WLPtr, out_queue: Pointer[WLPtr, MutUntrackedOrigin]) -> Int32:
    """Register the capture dispatcher on xdg_surface and write
    the queue handle to out_queue[0]. Pop events from that queue."""
    return _shim_listen(self, "xdg_surface", out_queue)

def xdg_surface_next_configure(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
...
def xdg_toplevel_listen(self: WLPtr, out_queue: Pointer[WLPtr, MutUntrackedOrigin]) -> Int32:
    """Register the capture dispatcher on xdg_toplevel and write
    the queue handle to out_queue[0]. Pop events from that queue."""
    return _shim_listen(self, "xdg_toplevel", out_queue)

def xdg_toplevel_next_configure(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
...
def xdg_toplevel_next_configure_bounds(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
    """Pop the next pending configure_bounds event into out_args (len 2). False = none."""
    var rc = _shim_event_pop(queue, 2, out_args)
    return rc == 2

def xdg_toplevel_next_wm_capabilities(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
...
def xdg_popup_listen(self: WLPtr, out_queue: Pointer[WLPtr, MutUntrackedOrigin]) -> Int32:
    """Register the capture dispatcher on xdg_popup and write
    the queue handle to out_queue[0]. Pop events from that queue."""
    return _shim_listen(self, "xdg_popup", out_queue)

def xdg_popup_next_configure(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
...
def xdg_popup_next_repositioned(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
