...
def wl_display_listen(self: WLPtr, out_queue: Pointer[WLPtr, MutUntrackedOrigin]) -> Int32:
...
def wl_display_next_error(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
    """Pop the next pending error event into out_args (len 3). False = none."""
    var rc = _shim_event_pop(queue, 0, out_args)
    return rc == 0

def wl_display_next_delete_id(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
...
def wl_registry_listen(self: WLPtr, out_queue: Pointer[WLPtr, MutUntrackedOrigin]) -> Int32:
    """Register the capture dispatcher on wl_registry and write
    the queue handle to out_queue[0]. Pop events from that queue."""
    return _shim_listen(self, "wl_registry", out_queue)

def wl_registry_next_global(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
    """Pop the next pending global event into out_args (len 3). False = none."""
    var rc = _shim_event_pop(queue, 0, out_args)
    return rc == 0

def wl_registry_next_global_remove(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
...
def wl_callback_listen(self: WLPtr, out_queue: Pointer[WLPtr, MutUntrackedOrigin]) -> Int32:
    """Register the capture dispatcher on wl_callback and write
    the queue handle to out_queue[0]. Pop events from that queue."""
    return _shim_listen(self, "wl_callback", out_queue)

def wl_callback_next_done(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
...
def wl_shm_listen(self: WLPtr, out_queue: Pointer[WLPtr, MutUntrackedOrigin]) -> Int32:
    """Register the capture dispatcher on wl_shm and write
    the queue handle to out_queue[0]. Pop events from that queue."""
    return _shim_listen(self, "wl_shm", out_queue)

def wl_shm_next_format(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
...
def wl_buffer_listen(self: WLPtr, out_queue: Pointer[WLPtr, MutUntrackedOrigin]) -> Int32:
...
def wl_data_offer_listen(self: WLPtr, out_queue: Pointer[WLPtr, MutUntrackedOrigin]) -> Int32:
    """Register the capture dispatcher on wl_data_offer and write
    the queue handle to out_queue[0]. Pop events from that queue."""
    return _shim_listen(self, "wl_data_offer", out_queue)

def wl_data_offer_next_offer(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
    """Pop the next pending offer event into out_args (len 1). False = none."""
    var rc = _shim_event_pop(queue, 0, out_args)
    return rc == 0

def wl_data_offer_next_source_actions(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
    """Pop the next pending source_actions event into out_args (len 1). False = none."""
    var rc = _shim_event_pop(queue, 1, out_args)
    return rc == 1

def wl_data_offer_next_action(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
...
def wl_data_source_listen(self: WLPtr, out_queue: Pointer[WLPtr, MutUntrackedOrigin]) -> Int32:
    """Register the capture dispatcher on wl_data_source and write
    the queue handle to out_queue[0]. Pop events from that queue."""
    return _shim_listen(self, "wl_data_source", out_queue)

def wl_data_source_next_target(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
    """Pop the next pending target event into out_args (len 1). False = none."""
    var rc = _shim_event_pop(queue, 0, out_args)
    return rc == 0

def wl_data_source_next_send(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
...
def wl_data_source_next_action(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
    """Pop the next pending action event into out_args (len 1). False = none."""
...
def wl_data_device_listen(self: WLPtr, out_queue: Pointer[WLPtr, MutUntrackedOrigin]) -> Int32:
...
def wl_data_device_next_enter(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
...
def wl_data_device_next_motion(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
...
def wl_data_device_next_selection(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
...
def wl_data_device_manager_get_data_device(self: WLPtr, seat: Pointer[NoneType, MutUntrackedOrigin]) raises -> WLPtr:
...
def wl_shell_get_shell_surface(self: WLPtr, surface: Pointer[NoneType, MutUntrackedOrigin]) raises -> WLPtr:
...
def wl_shell_surface_listen(self: WLPtr, out_queue: Pointer[WLPtr, MutUntrackedOrigin]) -> Int32:
    """Register the capture dispatcher on wl_shell_surface and write
    the queue handle to out_queue[0]. Pop events from that queue."""
    return _shim_listen(self, "wl_shell_surface", out_queue)

def wl_shell_surface_next_ping(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
    """Pop the next pending ping event into out_args (len 1). False = none."""
    var rc = _shim_event_pop(queue, 0, out_args)
    return rc == 0

def wl_shell_surface_next_configure(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
...
def wl_surface_listen(self: WLPtr, out_queue: Pointer[WLPtr, MutUntrackedOrigin]) -> Int32:
    """Register the capture dispatcher on wl_surface and write
    the queue handle to out_queue[0]. Pop events from that queue."""
    return _shim_listen(self, "wl_surface", out_queue)

def wl_surface_next_enter(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
    """Pop the next pending enter event into out_args (len 1). False = none."""
    var rc = _shim_event_pop(queue, 0, out_args)
    return rc == 0

def wl_surface_next_leave(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
    """Pop the next pending leave event into out_args (len 1). False = none."""
    var rc = _shim_event_pop(queue, 1, out_args)
    return rc == 1

def wl_surface_next_preferred_buffer_scale(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
    """Pop the next pending preferred_buffer_scale event into out_args (len 1). False = none."""
    var rc = _shim_event_pop(queue, 2, out_args)
    return rc == 2

def wl_surface_next_preferred_buffer_transform(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
...
def wl_seat_listen(self: WLPtr, out_queue: Pointer[WLPtr, MutUntrackedOrigin]) -> Int32:
    """Register the capture dispatcher on wl_seat and write
    the queue handle to out_queue[0]. Pop events from that queue."""
    return _shim_listen(self, "wl_seat", out_queue)

def wl_seat_next_capabilities(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
    """Pop the next pending capabilities event into out_args (len 1). False = none."""
    var rc = _shim_event_pop(queue, 0, out_args)
    return rc == 0

def wl_seat_next_name(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
...
def wl_pointer_listen(self: WLPtr, out_queue: Pointer[WLPtr, MutUntrackedOrigin]) -> Int32:
    """Register the capture dispatcher on wl_pointer and write
    the queue handle to out_queue[0]. Pop events from that queue."""
    return _shim_listen(self, "wl_pointer", out_queue)

def wl_pointer_next_enter(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
    """Pop the next pending enter event into out_args (len 4). False = none."""
    var rc = _shim_event_pop(queue, 0, out_args)
    return rc == 0

def wl_pointer_next_leave(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
    """Pop the next pending leave event into out_args (len 2). False = none."""
    var rc = _shim_event_pop(queue, 1, out_args)
    return rc == 1

def wl_pointer_next_motion(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
    """Pop the next pending motion event into out_args (len 3). False = none."""
    var rc = _shim_event_pop(queue, 2, out_args)
    return rc == 2

def wl_pointer_next_button(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
    """Pop the next pending button event into out_args (len 4). False = none."""
    var rc = _shim_event_pop(queue, 3, out_args)
    return rc == 3

def wl_pointer_next_axis(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
...
def wl_pointer_next_axis_source(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
    """Pop the next pending axis_source event into out_args (len 1). False = none."""
    var rc = _shim_event_pop(queue, 6, out_args)
    return rc == 6

def wl_pointer_next_axis_stop(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
    """Pop the next pending axis_stop event into out_args (len 2). False = none."""
    var rc = _shim_event_pop(queue, 7, out_args)
    return rc == 7

def wl_pointer_next_axis_discrete(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
    """Pop the next pending axis_discrete event into out_args (len 2). False = none."""
    var rc = _shim_event_pop(queue, 8, out_args)
    return rc == 8

def wl_pointer_next_axis_value120(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
    """Pop the next pending axis_value120 event into out_args (len 2). False = none."""
    var rc = _shim_event_pop(queue, 9, out_args)
    return rc == 9

def wl_pointer_next_axis_relative_direction(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
...
def wl_pointer_next_warp(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
...
def wl_keyboard_listen(self: WLPtr, out_queue: Pointer[WLPtr, MutUntrackedOrigin]) -> Int32:
    """Register the capture dispatcher on wl_keyboard and write
    the queue handle to out_queue[0]. Pop events from that queue."""
    return _shim_listen(self, "wl_keyboard", out_queue)

def wl_keyboard_next_keymap(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
    """Pop the next pending keymap event into out_args (len 3). False = none."""
    var rc = _shim_event_pop(queue, 0, out_args)
    return rc == 0

def wl_keyboard_next_enter(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
    """Pop the next pending enter event into out_args (len 3). False = none."""
    var rc = _shim_event_pop(queue, 1, out_args)
    return rc == 1

def wl_keyboard_next_leave(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
    """Pop the next pending leave event into out_args (len 2). False = none."""
    var rc = _shim_event_pop(queue, 2, out_args)
    return rc == 2

def wl_keyboard_next_key(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
    """Pop the next pending key event into out_args (len 4). False = none."""
    var rc = _shim_event_pop(queue, 3, out_args)
    return rc == 3

def wl_keyboard_next_modifiers(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
    """Pop the next pending modifiers event into out_args (len 5). False = none."""
    var rc = _shim_event_pop(queue, 4, out_args)
    return rc == 4

def wl_keyboard_next_repeat_info(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
...
def wl_touch_listen(self: WLPtr, out_queue: Pointer[WLPtr, MutUntrackedOrigin]) -> Int32:
    """Register the capture dispatcher on wl_touch and write
    the queue handle to out_queue[0]. Pop events from that queue."""
    return _shim_listen(self, "wl_touch", out_queue)

def wl_touch_next_down(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
    """Pop the next pending down event into out_args (len 6). False = none."""
    var rc = _shim_event_pop(queue, 0, out_args)
    return rc == 0

def wl_touch_next_up(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
    """Pop the next pending up event into out_args (len 3). False = none."""
    var rc = _shim_event_pop(queue, 1, out_args)
    return rc == 1

def wl_touch_next_motion(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
...
def wl_touch_next_shape(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
    """Pop the next pending shape event into out_args (len 3). False = none."""
    var rc = _shim_event_pop(queue, 5, out_args)
    return rc == 5

def wl_touch_next_orientation(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
...
def wl_output_listen(self: WLPtr, out_queue: Pointer[WLPtr, MutUntrackedOrigin]) -> Int32:
    """Register the capture dispatcher on wl_output and write
    the queue handle to out_queue[0]. Pop events from that queue."""
    return _shim_listen(self, "wl_output", out_queue)

def wl_output_next_geometry(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
    """Pop the next pending geometry event into out_args (len 8). False = none."""
    var rc = _shim_event_pop(queue, 0, out_args)
    return rc == 0

def wl_output_next_mode(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
...
def wl_output_next_scale(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
    """Pop the next pending scale event into out_args (len 1). False = none."""
    var rc = _shim_event_pop(queue, 3, out_args)
    return rc == 3

def wl_output_next_name(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
    """Pop the next pending name event into out_args (len 1). False = none."""
    var rc = _shim_event_pop(queue, 4, out_args)
    return rc == 4

def wl_output_next_description(queue: WLPtr, out_args: Pointer[WLArgument, MutUntrackedOrigin]) -> Bool:
...
def wl_subcompositor_get_subsurface(self: WLPtr, surface: Pointer[NoneType, MutUntrackedOrigin], parent: Pointer[NoneType, MutUntrackedOrigin]) raises -> WLPtr:
